...
def transform_map_to_numbers(elevation_map):
    elevation_dictionary = { "S" : 0, "E" : 25 } | { letter : i for i, letter in enumerate(string.ascii_lowercase)}
...
def get_edges(elevation_map):
    ...
    for n_row, row in enumerate(elevation_map):
        for n_col, current_node_elevation in enumerate(row):
            adjacent_indices = ((n_row, n_col - 1), (n_row, n_col + 1), (n_row - 1, n_col), (n_row + 1, n_col))
            for (other_row, other_col) in adjacent_indices:
                if 0 <= other_row < max_rows and 0 <= other_col < max_cols:
                    ...
                    if other_node_elevation - current_node_elevation <= 1:
                        yield ((n_row, n_col), (other_row, other_col), )
...
def part_two():
    ...
    starting_points = [ (row, col) for row in range(len(elevation_map)) for col in range(len(elevation_map[0])) if elevation_map[row][col] == "a" ]
    return min([ shortest_path_length(elevation_graph, starting_point, end) for  starting_point in starting_points ])
