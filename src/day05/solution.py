...
def parse_crate_state(crate_state_string):
    stack_numbers, *stacks = [ stack for stack in reversed(crate_state_string.split("\n")) ]
    stack_numbers = [ int(stack_number.strip()) for stack_number in stack_numbers.split("  ")]
    stacks = [ [stack[i:i+4].strip() for i in range(0, len(stack), 4)] for stack in stacks ]
    return stack_numbers, stacks
...
def create_stack_state(stack_numbers, stacks):
    stack_state = {i : list() for i in stack_numbers}
    for stack in stacks:
...
def part_one():
    ...
    return "".join( crate_stack[-1] for crate_stack in crate_state.values())
